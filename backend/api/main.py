...
def predict(request: PredictRequest) -> PredictResponse:
    ...
    def _apply_nr3_fcc_served_to_result(*, reuse_sidecar: dict | None = None) -> bool:
        nonlocal nr3_fcc_sidecar_diagnostics
        nonlocal nr3_fcc_served_applied
        nonlocal nr3_fcc_served_model_version
        nonlocal pipeline
        nonlocal fusion_note
        nonlocal blowout
        ...
        try:
            nr3_fcc_sidecar_diagnostics = reuse_sidecar or _run_nr3_fcc_sidecar()
    ...
    if xg_model_variant == "matchup_relative_v1":
        ...
        precomputed_sidecar: dict | None = None
        logger.warning(
        ...
        try:
            from core.live_nr3_fcc_shadow_runner import extract_served_prediction_from_sidecar
            ...
            nr3_reference_for_matchup: dict | None = None
            if config.nr3_fcc_served_enabled() and request.include_diagnostics:
                try:
                    precomputed_sidecar = _run_nr3_fcc_sidecar()
                    nr3_reference_for_matchup = extract_served_prediction_from_sidecar(
                        precomputed_sidecar
                    )
                except Exception:
                    logger.exception("matchup_nr3_reference_sidecar_failed")
            ...
            matchup_served = run_matchup_relative_v1_prediction(
                ...
                nr3_reference=nr3_reference_for_matchup,
            )
            ...
            if matchup_served.get("odds_blend_applied"):
                ...
                pipeline.odds_affect_prediction = True
            pipeline.final_probabilities_1x2 = dict(probs)
            if matchup_served.get("blowout_active"):
                blowout = BlowoutAdjustment(
                    home_xg=result["home_xg"],
                    away_xg=result["away_xg"],
                    alpha=request.alpha,
            ...
            if config.nr3_fcc_served_enabled():
                if _apply_nr3_fcc_served_to_result(reuse_sidecar=precomputed_sidecar):
                    model_variant_fallback = True
                    model_variant_fallback_reason = (
                        "matchup_relative_v1 failed; served NR3+FCC fallback applied"
                    )
        ...
        _apply_nr3_fcc_served_to_result()
